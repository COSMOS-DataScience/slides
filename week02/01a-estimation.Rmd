...
- Following the \emph{68--95--99.7\%} rule, with 0.95 probability, the
...
## Confidence intervals for the population mean}
- In reality, however, we usually have only one sample of $n$
...
we have a sample of $n=100$ people and that the sample mean is $\bar
{x}=123$. Therefore, we have one interval as follows:
$$\begin{equation*}
[123 - 3, 123 + 3] = [120, 126].
\end{equation*}$$
...
- Some times we can afford to be less confident, e.g., 68%:
$$\begin{equation*}
...
- If the random variable is not normally distributed, the sampling distribution of $\bar{X}$ can be considered as _approximately_ normal using (under certain conditions) the __central limit theorem (CLT)__:
...
- For the corresponding Bernoulli distribution, the mean is $\mu$ and the variance is $\mu(1-\mu)$. 
...
- Given the sample proportion $p$, the 95% confidence interval for the population proportion is obtained as follows:
\begin{equation*}
[p - 2 \times \mathit{SE}, p + 2 \times \mathit{SE}],
\end{equation*}
where
$$\begin{equation*}
\mathit{SE} = \sqrt{p(1-p)/n}.
\end{equation*}$$
