...
```{r echo = FALSE, message = FALSE}
alzheimer_data <- read.csv("/Users/zahramoslemi/Documents/Summer-2023/cosmos/alzheimer_data.csv")
```
...
## Probability Distribution of NACCICV Values in Alzheimer's Data:
...
Proportion of patients whose vol measurement is above or below 1200:
...
In NACC dataset, proportion of patients whose volume measurement is above 1200 is equal to `r proportion_above_1200`, and proportion of patients whose volume measurement is below 1000 is equal to `r proportion_below_1200`.
---
...
```{r eval=FALSE}
seq_num <- seq(1, 10, 1)
dis_samp <- sample(seq_num , 1000000, replace = TRUE)
dis_samp  <- as.data.frame(table(dis_samp))

ggplot(data = dis_samp , aes(x = dis_samp , y = Freq)) +
