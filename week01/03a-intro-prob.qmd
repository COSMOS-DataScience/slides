...
**Sample space**: the set of all possible outcomes of an experiment. Denoted by the symbol $\mathbf S$. Examples:

- For rolling a die, the sample space is $\mathbf S = \{1, 2, 3, 4, 5, 6\}$.
...
## Sample Space: Examples (cont.)
  - For measuring the resting heart rate of a randomly selected participant, the sample space is $\mathbf S = \{0, 1, 2, \ldots\}$ (assuming heart rate is measured in beats per minute).
  
  - For randomly selecting a patient and recording their blood type, the sample space is $\mathbf S = \{A, B, AB, O\}$. 
...
Sample space: $\mathbf S=\{1,2,3,4,5,6\}$
...
For $\mathbf S=\{1,2,3,4,5,6\}$,

- "Roll a 7" =$\emptyset$.

- "Roll a number between 1 and 6" = $\mathbf S$.
...
{\text{number of outcomes in }\mathbf S}.
$$
...
- Lex $X\sim U(a,b)$ where $a<b$. Then its pdf $f(x)$ is given by
...
# x-axis with arrow
arrows(0, 0, 10, 0, length = 0.08)

# Rectangle
rect(a, 0, b, h)

# Dashed vertical sides
segments(a, 0, a, h, lty = 2)
segments(b, 0, b, h, lty = 2)

# Labels
text(a, -0.012, "a")
text(b, -0.012, "b")
text(a - 0.55, h/2, expression(frac(1, b-a)))
```


## Example: Uniform Distribution $U(3,\,3.5)$

- Suppose Adam arrives at a coffee shop **at a random time** between **3:00 pm and 3:30 pm**.

- Every arrival time in this interval is equally likely.

- Let $X$ denote Adam's arrival time. Then
$$X \sim U(3,\,3.5).$$
...
arrows(2.8, 0, 3.7, 0, length = 0.08)
...
- the **probability** is the area between 3.1 and 3.3, which is a rectangle with width $3.3-3.1=0.20$ and height $2$. So the area is: $2\times0.20=0.40.$


## Probability and Area Under the PDF

For $X\si mU(3, 3.5)$, similarly, we can find the probability of any interval by calculating the area under the PDF. 

- $P(3.09<X<3.11)=2\times(3.11-3.09)=0.04$

- $P(3.099<X<3.101)=2\times(3.101-3.099)=0.004$

- Can you guess what $P(X=3.1)$ is?
...
- In the $X\sim U(3,\,3.5)$ example, the pdf is $f(x)=2$ for $3<x<3.5$.

- From the previous slide, $P(X=3.1)=0$.

- So $fx$ **cannot** be the probability that $X=x$.
...
A probability density function (pdf) has two key properties:

- The **total area** under the density curve is **1**.

- The probability that a continuous random variable falls within an interval is the **area under the density curve** over that interval.
